...
class Link(namedtuple('Link', 'word, word_index, parent, parent_index, dep_type')):
    """
    ...
    def get_parents(links, index):
        ...
        """
        return [link.parent_index for link in links if link.word_index == index]
    ...
    def __get_networkx_digraph(links):
        ...
        edges = []
        for link in links:
            edges.append((link.parent_index, link.word_index))
